...
- **Work:** a compact daily list with search, category filters, counts, and direct editing. Select visible entries to organize together. Click a title to expand its description. **Log tools** contains carry forward and undo.
- **Add work:** opens one entry workspace with **Write an update** and **Paste a list** choices. The editor and live table preview stay together. Save returns to Work; unfinished text and reviewed batches remain available through **Resume draft**.
- **Shift & crew:** choose 8, 10, or 12 hours and a start time to fill the end time, including overnight shifts. Custom end times remain editable. Named presets retain times, location, supervisor, optional lead, and selected crew. Choose a future date to prepare that day's shift. The app holds one shift record per date.
- **Report:** review blockers and next-shift notes, then download/share the PDF or copy the report. Crew shows only that shift's selected members and additional names; acting lead appears only when entered.
- **History:** reopen dates and export/import JSON backups. Imports preserve existing dates and add missing dates, with the crew library, presets, categories, and drafts included.
...
Choose **Add work → Paste a list**, then paste with Ctrl+V (Windows) or Cmd+V (Mac). Use the **Separate into** dropdown to choose the grouping:

- **One update per line:** for short production lists. Each line becomes a suggested title in the review.
- **Ticket IDs (spaces / commas):** `tk28493 tk83939 tk939393` becomes three entries with optional descriptions. Commas, semicolons, and new lines also work. For existing descriptions, put each on a separate line as `tk28493 | Checked link` (a tab or spaced dash also works). Unexpected text is left in the paste box with an error instead of being discarded. **Shared details for this list** opens the optional batch fields. **Fill empty fields** applies a shared description, category, or row/area only to blank fields in non-skipped updates. Review fields and unapplied shared values are saved as drafts.
...
To write one update yourself, choose **Write an update**, enter a **Title / ticket**, and add a description if useful. Switching views preserves both drafts. Both routes add to the same daily log.
...
## Crew library and Excel exchange

In **Shift & crew**, **Choose members** opens the reusable library. Search by name or email and filter by team / shift group. Check members for the selected date, then choose **Done**. The shift screen shows only that crew. Add members through the short form or import a CSV / cells copied from Excel. Names are required; work email and team are optional. A work email adds a clickable Teams chat link to the report and PDF, without creating an @mention or notification.

Crew tables use **Name, Email, Team** column headings. **Shift** is accepted as an alias for Team. CSV UTF-8, tab-separated Excel paste, semicolon-delimited CSV, quoted commas, quotes, accents, and multiline work descriptions are supported. Imports show a member checklist before saving. Matching emails, or matching names when no email is available, update the library. Use the optional checkbox to also add imported members to this shift. A CSV template is available inside Import crew. **Export this crew CSV** exports the current shift's names; **Export library CSV** exports all library members. Additional legacy crew text exports one name per line, without extracting emails from markup.

Each date and preset stores a crew snapshot. Editing or removing a library member does not rewrite existing shifts or reports. Remove someone from the current shift using the × beside their name. Library removal has a one-step Undo. Presets can be updated by saving the same name. **Use these details for future days** also retains the selected crew; new days always start with fresh work entries and blockers.

For work tables, **Report → Report options & text copy → Export work CSV** exports **Title, Description, Category, Area**. Import it through **Add work → Paste a list → Import work CSV**, or select **Work CSV table** and paste. This uses the existing review and matching-ticket choices before anything is added. Extra CSV columns are retained as labelled description text. Work CSV is a portable production summary: detailed ticket fields, quantities, and status become description text when exported. Use JSON backup for a complete, structured copy of the app. Work imports have the existing 12,000-character draft limit; split larger files into batches. Crew imports allow up to 2,000 library members and 200 selected members per shift.

CSV exports use a UTF-8 BOM, quoted fields, and spreadsheet-formula protection. Excel can open and edit them directly; save back as CSV UTF-8 to re-import. Native `.xlsx` files are not imported—copy cells or save as CSV. Imported crew and work stay in this browser; CSV files do not sync devices or send to Teams.
...
Version 4.0 reads schema 1–9 records and backups automatically, keeping the same storage key and website address. Reload to update; do not clear website data. Schema 9 adds custom categories, shift presets, duration choices, a reusable crew library, and crew snapshots for each date. Older app versions cannot open schema 9 backups.
...
Unsaved forms are marked **Not saved**. Previewing does not add or change log entries. Batch previews exclude skipped rows, require a save choice for matching tickets, and show field errors when a draft cannot yet be previewed. After saving or canceling, Work shows the saved list. The full Report includes shift details and blockers.

## Edit and organize the daily log

Use **Edit** on an entry to change its title, progress, category, or row/area directly in the log. **Save changes** (Ctrl/Cmd+Enter) commits the edit; **Cancel** keeps the saved entry. Detailed entries also have **Full details** for the original request, quantities, and cable breakdowns. Inline drafts stay available while changing views or dates in the current session; save or cancel before reloading or opening that day’s report.

Category and row/area are optional. Type your own category or choose a suggestion. Saved names are reusable across single entries, batches, direct edits, and bulk organization. Search matches titles, descriptions, tickets, categories, status, and areas. Filters change the Work view only; reports and counts still include the whole day. New entries suggest a category only when one work type is clear; ambiguous text stays uncategorized. Paste review lets you change the suggestion. Existing entries are not reclassified automatically. **Organize log** switches between All updates, By category, and By row/area (natural order, such as Row 2 before Row 10). Check entries, choose **Set category** and/or **Set row/area**, then **Apply to selected** to organize them together. Blank area clears the tag only when Set row/area is checked.
